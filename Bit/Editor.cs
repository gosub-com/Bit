...
namespace Gosub.Bit
{
    ...
    public partial class Editor:UserControl
    {
        ...
        FontInfo			mTokenNormal;
        FontInfo			mTokenReserved;
        FontInfo			mTokenReservedName;
        FontInfo			mTokenIdentifier;
        FontInfo			mTokenComment;
        ...
        public TokenLoc CursorLoc
        {
            ...
            set
            {
                ...
                if (value == mCursorLoc)
                    return;
                mCursorLoc = value;
                if (CursorLocChanged != null)
                    CursorLocChanged(this, mEventArgs);
                UpdateCursorBlinker();
            }
        }
        ...
        /// </summary>
        FontInfo GetFontInfo(eTokenType tokenType)
        {
            switch (tokenType)
            {
                default:
                ...
                case eTokenType.Reserved:
                    return mTokenReserved;
                case eTokenType.ReservedName:
                    return mTokenReservedName;
        ...
        void DrawToken(Graphics gr, Token token, bool background)
        {
            ...
            if (gr != null)
            {
                ...
                else
                {
                    ...
                    // Draw the token
                    FontInfo fontInfo = GetFontInfo(token.Type);
                    gr.DrawString(token.Name, fontInfo.Font, fontInfo.ForeColor, x-TEXT_OFFSET_X, y, mTabFormat);
                }
            }
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (mFontSize == new SizeF())
            {
                ...
                // Setup fonts
                mTokenNormal = new FontInfo(normalFont, Brushes.Black);
                mTokenReserved = new FontInfo(boldFont, Brushes.Blue);
                mTokenReservedName = new FontInfo(normalFont, Brushes.Blue);
                mTokenIdentifier = new FontInfo(normalFont, Brushes.Black);
                mTokenComment = new FontInfo(normalFont, Brushes.Green);

                // Measure the font size
                SizeF size1 = e.Graphics.MeasureString("MM\r\nMM", mTokenNormal.Font);
                SizeF size2 = e.Graphics.MeasureString("MMM\r\nMMM\r\nMMM", mTokenNormal.Font);
            ...
            if (MouseHoverToken != null 
                && (MouseHoverToken.Type == eTokenType.Identifier
                    || MouseHoverToken.Type == eTokenType.ReservedName))
            {
            ...
            if (mCursorVisible)
            {
                        ...
                        && CursorLoc.Char < GetLine(CursorLoc.Line).Length)
                {
                    ...
                    e.Graphics.DrawString(GetLine(CursorLoc.Line)[CursorLoc.Char].ToString(),
                                            mTokenNormal.Font, Brushes.White,
                                            x - TEXT_OFFSET_X, y);
                }
            }
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (mTestToken != mLastMouseHoverToken)
            {
                ...
                if (MouseHoverTokenChanged != null)
                    MouseHoverTokenChanged(previousToken, mLastMouseHoverToken);
                Invalidate();
            }
        ...
        class FontInfo
        {
            ...
            public FontInfo(Font font, Brush foreColor)
            {
                Font = font;
                ForeColor = foreColor;
            }
